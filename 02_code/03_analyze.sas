...
FILES IN        sust.sustain_final - Do not edit.
FILES OUT       
...
libname sust "&path./01_SAS_SourceCode_Datasets";
...
PROC MIXED DATA = sust.sustain_final  noclprint covtest;
...
title "DV: BH, lsmeans = location";
    RUN; *find ICC by dividing cov param est UN(1,1) by residual (.1626/.4289 = .379 which is really high per MD);
...
proc genmod data=sust.sustain_final_ORD;
   class  pracid    location orgTyp ageTyp size;
   model  Bh3lev = location orgTyp ageTyp  size/ dist=multinomial
                         link=cumlogit;
    repeated subject=pracid / printmle;
    ods output ParameterEstimates = sust.gmparms_BH3level;
        title "proc genmod DV Bh3level";
run;

/*proc genmod data=sust.sustain_final1;*/
/*   class  pracid    location orgTyp ageTyp size;*/
...
----------------------------------------------------------------------------------*/
/*look for proportion*/
/*proc freq data = sust.sustain_final_ORD;*/
/*tables HIT: ;*/
...
proc genmod data=sust.sustain_final_ORD;
   ...
   model  HIT3lev = location orgTyp ageTyp  size/ dist=multinomial
                         link=cumlogit;
    repeated subject=pracid /printmle;
    ods output ParameterEstimates = sust.gmparms_HIT3level;
...
proc genmod data=sust.sustain_final_ORD;
   ...
   model  Addtl3lev = location orgTyp ageTyp  size/ dist=multinomial
                         link=cumlogit;
    repeated subject=pracid /printmle;
    ods output ParameterEstimates = sust.gmparms_Addtl3level;
        title "Parameter Estimates: proc genmod DV HIT 3-levels";
run;
